...
app.controller("mainController", ($scope, $http, $location, $interval, Config, UserService) =>
{
    ...
    $scope.checkTokenInterval = $interval(() => { checkToken($http, Config).catch(error => { if($scope.User) $scope.logout(); }); }, 2*60*1000);
    ...
    $scope.$on("$routeChangeSuccess", () =>
    {
        ...
        checkToken($http, Config).catch(error => $scope.logout());
    });
